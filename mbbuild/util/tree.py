...
class Tree:
    def __init__(self,c='',ch=None, p=None, l=0, r=0):
        if ch is None:
            ch = []
        self.c  = c
        self.ch = ch
    ...
    def prune(self, criterion=None):
        if criterion is not None:
            assert hasattr(criterion, '__call__'), 'Pruning criterion must be a callable with one argument (the tree).'
            if criterion(self):
                self.drop()
                pruned = True
            elif len(self.ch) > 0:
                i = 0
                while i < len(self.ch):
                    ch_pruned = self.ch[i].prune(criterion=criterion)
                    i += (1 - ch_pruned)
                if len(self.ch) == 0:
                    self.drop()
                    pruned = True
    ...
    def mapLabels(self, labelmap=None):
        if map is not None:
            self.c = labelmap(self.c)
            for x in self.ch:
                x.mapLabels(labelmap)
