...
python3 mbbuild/static_resources/scripts/merge_tables.py $prdmeasures $resmeasures subject docid sentid sentpos word > $tblfile;
python3 mbbuild/static_resources/scripts/check_preds.py $tblfile $bform $preds_add_STR $preds_ablate_STR;
mbbuild/static_resources/scripts/regress-lmer.R <(cat $tblfile) $outfile -b $bform $preds_add_STR $preds_ablate_STR -c $corpusname -e;
rm $tblfile;
