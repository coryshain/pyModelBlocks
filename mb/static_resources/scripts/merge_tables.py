...
if __name__ == '__main__':
    argparser = argparse.ArgumentParser(description='Merges two space-delimited data tables.')
    argparser.add_argument('f1', type=str, help='Path to first input data table')
    argparser.add_argument('f2', type=str, help='Path to second input data table')
    argparser.add_argument('key_cols', metavar='key', type=str, nargs='+', help='Merge key fields')
    ...
    data1 = pd.read_csv(args.f1, sep=' ', skipinitialspace=True)
    data2 = pd.read_csv(args.f2, sep=' ', skipinitialspace=True)
